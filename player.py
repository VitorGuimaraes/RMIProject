...
# Inverte o tabuleiro recebido, ex:
# [True, True, None, False, False] vira [False, False, None, True, True]
def invert_board():
	global board
...
def write_in_chat(event, screen):
	...
	if event.key == K_BACKSPACE:
		...
		gameserver.send(im_player, chat) # Envia a mensagem para o outro jogador
...
# Informa na tela se o jogador perdeu ou venceu
		...
		for event in pg.event.get():
			if event.type == pg.MOUSEBUTTONDOWN:
				...
				if x > 552 and x < 749 and y > 530 and y < 582:
					gameserver.send(im_player, answer)
					accepted_resquest = True

				decide = True
			
			elif event.type == QUIT: # Fechar o jogo
				gameserver.send(im_player, "<]f") # Notifica o servidor que o jogo foi fechado
				return
...
# Recebe os pacotes vindos do servidor
def receive_packet():
	global vez
	global opponent_ask_surrender
	global opponent_ask_restart
	...
	while True:
		...
		msg = gameserver.request(im_player, "msg")    # Aguarda receber mensagens do servidor
		if msg != None:
			...
			gameserver.reset_msg_controller(im_player)
...
def main():
	...
	gameserver = Pyro4.Proxy("PYRONAME:ponghau.gameserver")
	# Invoca método remoto do servidor para saber qual player será
	im_player = gameserver.you_are_player()
